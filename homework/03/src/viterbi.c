/*
 * Occasionally Dishonest Casino - Sequence Prediction via Viterbi Algorithm
 *
 * Description:
...
int main(int argc, char **argv) {
	...
	//First states
	dp[F][0] = log2(0.5) + log2(HMM[F][l2id(seq[0])]);
	dp[L][0] = log2(0.5) + log2(HMM[L][l2id(seq[0])]);

	//Now then... the others.
	for (i = 1; i < len; i++) {
		//Free
		dp[F][i] = log2(HMM[F][l2id(seq[i])]) + maxd_ext(
			log2(swp[F][F]) + dp[F][i - 1],
			log2(swp[L][F]) + dp[L][i - 1],
			&ch[F][i]
		);
