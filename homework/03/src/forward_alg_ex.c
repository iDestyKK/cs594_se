/*
 * Forward Algorithm - A toy example
 *
 * Description:
...
int main(int argc, char **argv) {
	...
	//First states
	dp[H][0] = 0.5 * HMM[H][l2id(seq[0])];
	dp[L][0] = 0.5 * HMM[L][l2id(seq[0])];
	...
	for (i = 1; i < len; i++) {
		...
		*/
		...
		//math... lol
		dp[H][i] =
			(dp[H][i - 1] * swp[H][H] * HMM[H][l2id(seq[i])]) +
			(dp[L][i - 1] * swp[L][H] * HMM[H][l2id(seq[i])]);

		dp[L][i] =
			(dp[L][i - 1] * swp[L][L] * HMM[L][l2id(seq[i])]) +
			(dp[H][i - 1] * swp[H][L] * HMM[L][l2id(seq[i])]);
	}

	dump_table(dp, ch, len);

	printf("%lg\n", dp[H][len - 1] + dp[L][len - 1]);
